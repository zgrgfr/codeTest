 ...
 *
 *
 * Created by guofengrui on 2017/5/23.
 */
    ...
    public boolean match2(char[] str, int strIndex, char[] pattern, int patternIndex) {
        ...
        if (patternIndex + 1 < pattern.length && pattern[patternIndex + 1] == '*') {
            if ((strIndex != str.length && pattern[patternIndex] == str[strIndex]) || (pattern[patternIndex] == '.' && strIndex != str.length)) {
                return match2(str, strIndex, pattern, patternIndex + 2)// 模式后移2，视为x*匹配0个字符
                        || match2(str, strIndex + 1, pattern, patternIndex + 2)// 视为模式匹配1个字符
                        || match2(str, strIndex + 1, pattern, patternIndex);// *匹配1个，再匹配str中的下一个
            } else {
